...
def get_output_filename(dataset_path):
    ...
    return f"outputs/JSON_ENB_scaled/grad_{first_three}_{num_layers}S_{blocks_per_layer}B_{output_blocks}L_scaled_10max.json"
...
def scale_features_minmax(X, min_positive=MIN_POSITIVE):
    """
    Apply MinMaxScaler to features with range [0.1, 10].
    """
    arr = np.asarray(X, dtype=float).copy()
    
    print(f"   📊 Data range: [{np.min(arr):.3f}, {np.max(arr):.3f}]")
    print(f"   🎯 Scaling to: [0.1, 10]")
    
    # Apply MinMaxScaler to scale features to [0.1, 10] range
...
def main():
    ...
    for fold, (tr, te) in enumerate(fold_splits, 1):
        ...
        x_train_s = scale_features_minmax(X_train)  # MinMaxScaler to [0.1, 10] range
