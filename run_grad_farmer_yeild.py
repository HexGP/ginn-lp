...
# =============== USER CONFIG ===============
DATA_CSV = "data/prepared/farmer_yield_sustainability.csv"   # <--- farmer yield sustainability dataset
...
def get_output_filename(dataset_path):
    ...
    return f"outputs/JSON_farmer_yield/grad_{first_three}_{num_layers}S_{blocks_per_layer}B_{output_blocks}L_shifted.json"

# If you know exact target col names, set them here (otherwise auto-detect below).
TARGET_COLS = ["target_Crop_Yield_ton", "target_Sustainability_Score"]  # Farmer yield sustainability targets
...
def main():
    # 1) Load data (farmer yield sustainability dataset)
    ...
    # 12) Save results
    os.makedirs("outputs/JSON_farmer_yield", exist_ok=True)
