#!/usr/bin/env python3
"""
Quick script to test REFITTED equations from ENB dataset on test data.
This tests the equations that were refitted on training data (should be no data leakage).
"""
...
def main():
    ...
    print("=== Testing REFITTED Equations from ENB Dataset ===\n")
    ...
    for target_idx, target_data in enumerate(fold_data['per_target']):
        ...
        # Get the REFITTED equation expression
        refit_expr = target_data['expr_refit']
        print(f"Refitted Equation: {refit_expr[:100]}...")
