...
(one-file runner; no scaling; selective Savitzky–Golay smoothing + positivity clamp)
...
What this does:
  ...
  • Every N epochs:
      ...
      - Optionally refits ONLY numeric constants in the printed equations
        to better match your (selectively smoothed) data (structure/exponents fixed)
      - Reports R²/MAE/RMSE and faithfulness R²(model ↔ equation)
  • No scaling; uses SELECTIVE Savitzky–Golay smoothing (only on columns with zeros/negatives) + min-positive clamp.
...
def get_output_filename(dataset_path):
    ...
    return f"ginn_grad_{first_three}_sim.json"
...
# Selective smoothing configuration
USE_SELECTIVE_SMOOTHING = True        # Use selective smoothing (only on columns with zeros/negatives)
SMOOTHING_WINDOW = 15                 # Window length for Savitzky-Golay filter
SMOOTHING_POLYORDER = 3               # Polynomial order for Savitzky-Golay filter
ZERO_THRESHOLD = 1e-10                # Threshold below which values are considered "zero"
...
def selective_savgol_positive(X, window_length=15, polyorder=3, min_positive=MIN_POSITIVE, 
                             zero_threshold=1e-10, verbose=True):
    """
    Apply Savitzky-Golay smoothing ONLY to columns that contain zeros or negative values.
    Preserves natural noise and distribution for columns that are already positive.
    
    Args:
        X: Input array (features or targets)
        window_length: Window length for Savitzky-Golay filter
        polyorder: Polynomial order for Savitzky-Golay filter
        min_positive: Minimum positive value to enforce
        zero_threshold: Threshold below which values are considered "zero"
        verbose: Whether to print which columns are being smoothed
    
    Returns:
        Processed array with selective smoothing applied
    """
    arr = np.asarray(X, dtype=float).copy()
    n, d = arr.shape
    
    if verbose:
        print(f"[SelectiveSmoothing] Analyzing {d} columns for zeros/negatives...")
    
    # Identify columns that need smoothing (contain zeros or negatives)
    columns_to_smooth = []
    for j in range(d):
        col_data = arr[:, j]
        has_zeros_or_negatives = np.any(col_data <= zero_threshold)
        
        if has_zeros_or_negatives:
            columns_to_smooth.append(j)
            if verbose:
                min_val = np.min(col_data)
                max_val = np.max(col_data)
                zero_count = np.sum(col_data <= zero_threshold)
                print(f"[SelectiveSmoothing] Column {j}: NEEDS smoothing (min={min_val:.6f}, max={max_val:.6f}, zeros/negatives={zero_count})")
        else:
            if verbose:
                min_val = np.min(col_data)
                max_val = np.max(col_data)
                print(f"[SelectiveSmoothing] Column {j}: PRESERVING natural distribution (min={min_val:.6f}, max={max_val:.6f})")
    
    # Apply smoothing only to identified columns
    if columns_to_smooth:
        wl = max(3, min(window_length, (n // 2) * 2 + 1))  # must be odd, <= n and >=3
        if verbose:
            print(f"[SelectiveSmoothing] Applying Savitzky-Golay smoothing to {len(columns_to_smooth)} columns: {columns_to_smooth}")
        
        for j in columns_to_smooth:
            if n >= wl:
                arr[:, j] = savgol_filter(arr[:, j], wl, polyorder)
                if verbose:
                    print(f"[SelectiveSmoothing] Smoothed column {j}")
    
    # Apply positivity clamp to ALL columns (for numerical stability)
    arr = np.where(np.isfinite(arr), arr, 0.0)
    arr = np.sign(arr) * np.maximum(np.abs(arr), EPS_LAURENT)   # avoid exact 0
    arr = np.maximum(arr, min_positive)                         # enforce positive domain
    
    if verbose:
        print(f"[SelectiveSmoothing] Applied positivity clamp to all columns (min_positive={min_positive})")
...
def main():
    ...
    for fold, (tr, te) in enumerate(fold_splits, 1):
        ...
        # 3) Selective smoothing (Savitzky–Golay) + positivity clamp; NO SCALING
        # Apply smoothing ONLY to columns with zeros/negatives, preserve natural noise for others
        if USE_SELECTIVE_SMOOTHING:
            print(f"\n🔧 Using SELECTIVE smoothing (window={SMOOTHING_WINDOW}, polyorder={SMOOTHING_POLYORDER})")
            print(f"   • Smoothing only columns with zeros/negatives (threshold={ZERO_THRESHOLD})")
            print(f"   • Preserving natural distribution for positive-only columns")
            
            X_train_s = selective_savgol_positive(
                X_train, 
            ...
            Y_train_s = selective_savgol_positive(
                Y_train, 
                window_length=SMOOTHING_WINDOW, 
                polyorder=SMOOTHING_POLYORDER,
                zero_threshold=ZERO_THRESHOLD,
            ...
            X_test_s = selective_savgol_positive(
                X_test, 
                window_length=SMOOTHING_WINDOW, 
                polyorder=SMOOTHING_POLYORDER,
                zero_threshold=ZERO_THRESHOLD,
            ...
            Y_test_s = selective_savgol_positive(
                Y_test, 
                window_length=SMOOTHING_WINDOW, 
                polyorder=SMOOTHING_POLYORDER,
                zero_threshold=ZERO_THRESHOLD,
            ...
            # Fallback to original smoothing for all columns
            print(f"\n🔧 Using STANDARD smoothing (window={SMOOTHING_WINDOW}, polyorder={SMOOTHING_POLYORDER})")
            print(f"   • Applying smoothing to ALL columns")
            
            X_train_s = savgol_positive(X_train, window_length=SMOOTHING_WINDOW, polyorder=SMOOTHING_POLYORDER)
            Y_train_s = savgol_positive(Y_train, window_length=SMOOTHING_WINDOW, polyorder=SMOOTHING_POLYORDER)
            X_test_s = savgol_positive(X_test, window_length=SMOOTHING_WINDOW, polyorder=SMOOTHING_POLYORDER)
            Y_test_s = savgol_positive(Y_test, window_length=SMOOTHING_WINDOW, polyorder=SMOOTHING_POLYORDER)
