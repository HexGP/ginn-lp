...
class MLPRegressionModel:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, feature_names: List[str] = None, 
        ...
        self.n_parameters_ = sum(coef.size + intercept.size for coef, intercept in zip(self.model.coefs_, self.model.intercepts_))
...
def plot_results(y_true: np.ndarray, y_pred: np.ndarray, target_names: List[str], model_name: str):
    ...
    plt.tight_layout()
    plt.savefig(f'{model_name.lower()}_mlp_results.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def plot_training_loss(model: MLPRegressionModel, model_name: str):
    ...
    plt.tight_layout()
    plt.savefig(f'{model_name.lower()}_training_loss.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def main():
    ...
    try:
        ...
        final_model = MLPRegressionModel(
            hidden_layer_sizes=best_estimator.hidden_layer_sizes,
            activation=best_estimator.activation,
        ...
        results_df.to_csv('tuned_mlp_results.csv')
        print(f"\nTuned model results saved to 'tuned_mlp_results.csv'")
    ...
    results_df.to_csv(f'{best_arch_name.lower().replace(" ", "_").replace("(", "").replace(")", "").replace(",", "")}_results.csv')
    print(f"\nResults saved to '{best_arch_name.lower().replace(' ', '_').replace('(', '').replace(')', '').replace(',', '')}_results.csv'")
    
    # Save comparison results
    comparison_df = pd.DataFrame(comparison_results).T
    comparison_df.to_csv('mlp_architecture_comparison.csv')
    print(f"Architecture comparison results saved to 'mlp_architecture_comparison.csv'")
