...
class MultiOutputRegressionModel:
    ...
    def _initialize_estimator(self):
        """Initialize the base estimator based on the specified type."""
    ...
    def get_feature_importance(self) -> Dict[str, np.ndarray]:
        ...
        if not hasattr(self.model.estimators_[0], 'feature_importances_'):
            raise ValueError("Base estimator does not support feature importance")
            
        importance_dict = {}
        for i, target_name in enumerate(self.target_names):
            importance_dict[target_name] = self.model.estimators_[i].feature_importances_
    ...
    def get_coefficients(self) -> Dict[str, np.ndarray]:
        ...
        if not hasattr(self.model.estimators_[0], 'coef_'):
            raise ValueError("Base estimator does not support coefficients")
            
        coefficients_dict = {}
        for i, target_name in enumerate(self.target_names):
            coefficients_dict[target_name] = self.model.estimators_[i].coef_
...
def plot_results(y_true: np.ndarray, y_pred: np.ndarray, target_names: List[str], model_name: str):
    ...
    plt.tight_layout()
    plt.savefig(f'{model_name.lower()}_regression_results.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def main():
    ...
    if 'Random Forest' in best_model_name:
        ...
        estimator_type = 'random_forest'  # Default
    ...
    # For multi-output regression, we need to use the full y array
    # MSE cross-validation for the entire multi-output model
    mse_scores = cross_val_score(
        model.model, X, y, 
        cv=5, scoring='neg_mean_squared_error'
    )
    cv_scores_mse['overall'] = -mse_scores.mean()
    
    # MAPE cross-validation (using custom scoring for multi-output)
    ...
    cv_scores_mape['overall'] = -mape_scores.mean()
    
    # Individual target cross-validation using base estimators
    for i, target_name in enumerate(target_names):
        # Use the base estimator directly for individual target CV
        base_estimator = model.model.estimators_[i]
        
        # MSE cross-validation
        mse_scores = cross_val_score(
            base_estimator, X, y[:, i], 
            cv=5, scoring='neg_mean_squared_error'
        )
        cv_scores_mse[target_name] = -mse_scores.mean()
        
        # MAPE cross-validation
    ...
    # Print overall multi-output results
    print(f"\n{'OVERALL MULTI-OUTPUT MODEL':^60}")
    print("-" * 60)
    print(f"  CV MSE:  {cv_scores_mse['overall']:.4f}")
    print(f"  CV MAPE: {cv_scores_mape['overall']:.4f}")
    ...
    results_df.to_csv(f'{best_model_name.lower().replace(" ", "_")}_results.csv')
    print(f"\nResults saved to '{best_model_name.lower().replace(' ', '_')}_results.csv'")
