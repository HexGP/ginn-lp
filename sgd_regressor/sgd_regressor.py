...
class MultiTaskSGDRegressor:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, feature_names: List[str] = None, 
        ...
        # Fit separate SGD regressor model for each target
...
def plot_results(y_true: np.ndarray, y_pred: np.ndarray, target_names: List[str]):
    ...
    plt.tight_layout()
    plt.savefig('sgd_regression_results.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def main():
    ...
    for i, target_name in enumerate(target_names):
        ...
        mse_scores = cross_val_score(
            SGDRegressor(random_state=42, max_iter=1000, tol=1e-3), X, y[:, i], 
    ...
    results_df.to_csv('sgd_regression_results.csv')
    print(f"\nResults saved to 'sgd_regression_results.csv'")
    
    # Save comparison results
    comparison_df = pd.DataFrame(comparison_results).T
    comparison_df.to_csv('sgd_vs_linear_comparison.csv')
    print(f"Comparison results saved to 'sgd_vs_linear_comparison.csv'")
