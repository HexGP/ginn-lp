...
# =============== USER CONFIG ===============
DATA_CSV = "data/prepared/farmer_soil_conditions.csv"   # <--- farmer soil conditions dataset
...
def get_output_filename(dataset_path):
    ...
    return f"outputs/JSON_farmer_soil/grad_{first_three}_{num_layers}S_{blocks_per_layer}B_{output_blocks}L_shifted.json"

# If you know exact target col names, set them here (otherwise auto-detect below).
TARGET_COLS = ["target_Soil_pH", "target_Soil_Moisture"]  # Farmer soil conditions targets
...
def main():
    # 1) Load data (farmer soil conditions dataset)
    ...
    # 12) Save results
    os.makedirs("outputs/JSON_farmer_soil", exist_ok=True)
