...
def simplify_ginn_equations_with_pysr(X, Y, ginn_equations, target_names):
    ...
    for i, (ginn_eq, target_name) in enumerate(zip(ginn_equations, target_names)):
        ...
        print(f"GINN predictions mean: {ginn_predictions.mean():.6f}, std: {ginn_predictions.std():.6f}")
        
        # Check if GINN predictions are too constant-like
        if ginn_predictions.std() < 0.1:
            print(f"Warning: GINN predictions have very low variance ({ginn_predictions.std():.6f})")
            print("This might cause PySR to converge to constants")
        ...
        if best_equation.equals(model.equations_.iloc[0]):
...
def display_equations_and_results(Y_true, Y_original, ginn_equations, simplified_equations, X_scaled, X_original, target_names, scaler_Y):
    """Display equations first, then comprehensive results with both scaled and original scales."""
    
    # 1. Display GINN equations
    print("\n" + "="*80)
    print("GINN EXTRACTED EQUATIONS")
    print("="*80)
    for i, target_name in enumerate(target_names):
        print(f"\n{target_name}:")
        print(f"Complexity: High (multiple terms, exponents)")
        print(f"Equation: Complex multivariate equation with all 8 features")
    ...
    # 3. Evaluate both equations on both scales
    print("\n" + "="*80)
    print("EVALUATING EQUATIONS ON BOTH SCALES")
    print("="*80)
    
    # Initialize results storage
    results = {
        'ginn_scaled': {'y1_mse': 0, 'y2_mse': 0, 'y1_mape': 0, 'y2_mape': 0},
        'ginn_original': {'y1_mse': 0, 'y2_mse': 0, 'y1_mape': 0, 'y2_mape': 0},
        'pysr_scaled': {'y1_mse': 0, 'y2_mse': 0, 'y1_mape': 0, 'y2_mape': 0},
        'pysr_original': {'y1_mse': 0, 'y2_mse': 0, 'y1_mape': 0, 'y2_mape': 0}
    }
    
    for i, target_name in enumerate(target_names):
        print(f"\n--- Evaluating {target_name} ---")
        
        # Get GINN predictions on both scales
        ginn_preds_scaled = ginn_equations[i](X_scaled)
        
        # For inverse transform, we need to create a dummy array with zeros for the other output
        dummy_array = np.zeros((len(ginn_preds_scaled), 2))
        dummy_array[:, i] = ginn_preds_scaled
        ginn_preds_original = scaler_Y.inverse_transform(dummy_array)[:, i]
        
        # Get PySR predictions on both scales
        pysr_preds_scaled = simplified_equations[i]['predictions'] if 'predictions' in simplified_equations[i] else None
        if pysr_preds_scaled is None:
            ...
            pysr_preds_scaled = pysr_func(X_scaled)
            
            # Ensure pysr_preds_scaled is an array
            if np.isscalar(pysr_preds_scaled):
                pysr_preds_scaled = np.full(X_scaled.shape[0], pysr_preds_scaled)
        
        # For inverse transform, we need to create a dummy array with zeros for the other output
        dummy_array = np.zeros((len(pysr_preds_scaled), 2))
        dummy_array[:, i] = pysr_preds_scaled
        pysr_preds_original = scaler_Y.inverse_transform(dummy_array)[:, i]
        
        # Calculate metrics for scaled data
        ginn_mse_scaled = mean_squared_error(Y_true[:, i], ginn_preds_scaled)
        ginn_mape_scaled = mean_absolute_percentage_error(Y_true[:, i], ginn_preds_scaled)
        pysr_mse_scaled = mean_squared_error(Y_true[:, i], pysr_preds_scaled)
        pysr_mape_scaled = mean_absolute_percentage_error(Y_true[:, i], pysr_preds_scaled)
        
        # Calculate metrics for original data
        ginn_mse_original = mean_squared_error(Y_original[:, i], ginn_preds_original)
        ginn_mape_original = mean_absolute_percentage_error(Y_original[:, i], ginn_preds_original)
        pysr_mse_original = mean_squared_error(Y_original[:, i], pysr_preds_original)
        pysr_mape_original = mean_absolute_percentage_error(Y_original[:, i], pysr_preds_original)
        
        # Store results
        if i == 0:  # Y1
            results['ginn_scaled']['y1_mse'] = ginn_mse_scaled
            results['ginn_scaled']['y1_mape'] = ginn_mape_scaled
            results['ginn_original']['y1_mse'] = ginn_mse_original
            results['ginn_original']['y1_mape'] = ginn_mape_original
            results['pysr_scaled']['y1_mse'] = pysr_mse_scaled
            results['pysr_scaled']['y1_mape'] = pysr_mape_scaled
            results['pysr_original']['y1_mse'] = pysr_mse_original
            results['pysr_original']['y1_mape'] = pysr_mape_original
        else:  # Y2
            results['ginn_scaled']['y2_mse'] = ginn_mse_scaled
            results['ginn_scaled']['y2_mape'] = ginn_mape_scaled
            results['ginn_original']['y2_mse'] = ginn_mse_original
            results['ginn_original']['y2_mape'] = ginn_mape_original
            results['pysr_scaled']['y2_mse'] = pysr_mse_scaled
            results['pysr_scaled']['y2_mape'] = pysr_mape_scaled
            results['pysr_original']['y2_mse'] = pysr_mse_original
            results['pysr_original']['y2_mape'] = pysr_mape_original
        
        print(f"Scaled Data - GINN: MSE={ginn_mse_scaled:.6f}, MAPE={ginn_mape_scaled:.2f}%")
        print(f"Scaled Data - PySR: MSE={pysr_mse_scaled:.6f}, MAPE={pysr_mape_scaled:.2f}%")
        print(f"Original Data - GINN: MSE={ginn_mse_original:.6f}, MAPE={ginn_mape_original:.2f}%")
        print(f"Original Data - PySR: MSE={pysr_mse_original:.6f}, MAPE={pysr_mape_original:.2f}%")
...
def display_final_tables(results):
    ...
    print("-"*60)
    print(f"{'Model':<12} {'Scale':<10} {'Y1_MSE':<12} {'Y2_MSE':<12}")
    print("-" * 60)
    print(f"{'GINN':<12} {'Scaled':<10} {results['ginn_scaled']['y1_mse']:<12.6f} {results['ginn_scaled']['y2_mse']:<12.6f}")
    print(f"{'GINN':<12} {'Original':<10} {results['ginn_original']['y1_mse']:<12.6f} {results['ginn_original']['y2_mse']:<12.6f}")
    print(f"{'PySR':<12} {'Scaled':<10} {results['pysr_scaled']['y1_mse']:<12.6f} {results['pysr_scaled']['y2_mse']:<12.6f}")
    print(f"{'PySR':<12} {'Original':<10} {results['pysr_original']['y1_mse']:<12.6f} {results['pysr_original']['y2_mse']:<12.6f}")
    
    # Table 2: MAPE Results
    print("\n" + "-"*60)
    print("TABLE 2: MAPE RESULTS")
    print("-"*60)
    print(f"{'Model':<12} {'Scale':<10} {'Y1_MAPE':<12} {'Y2_MAPE':<12}")
    print("-" * 60)
    print(f"{'GINN':<12} {'Scaled':<10} {results['ginn_scaled']['y1_mape']:<12.2f} {results['ginn_scaled']['y2_mape']:<12.2f}")
    print(f"{'GINN':<12} {'Original':<10} {results['ginn_original']['y1_mape']:<12.2f} {results['ginn_original']['y2_mape']:<12.2f}")
    print(f"{'PySR':<12} {'Scaled':<10} {results['pysr_scaled']['y1_mape']:<12.2f} {results['pysr_scaled']['y2_mape']:<12.2f}")
    print(f"{'PySR':<12} {'Original':<10} {results['pysr_original']['y1_mape']:<12.2f} {results['pysr_original']['y2_mape']:<12.2f}")
    ...
    ginn_scaled_overall_mse = (results['ginn_scaled']['y1_mse'] + results['ginn_scaled']['y2_mse']) / 2
    ginn_scaled_overall_mape = (results['ginn_scaled']['y1_mape'] + results['ginn_scaled']['y2_mape']) / 2
    ginn_original_overall_mse = (results['ginn_original']['y1_mse'] + results['ginn_original']['y2_mse']) / 2
    ginn_original_overall_mape = (results['ginn_original']['y1_mape'] + results['ginn_original']['y2_mape']) / 2
    pysr_scaled_overall_mse = (results['pysr_scaled']['y1_mse'] + results['pysr_scaled']['y2_mse']) / 2
    pysr_scaled_overall_mape = (results['pysr_scaled']['y1_mape'] + results['pysr_scaled']['y2_mape']) / 2
    pysr_original_overall_mse = (results['pysr_original']['y1_mse'] + results['pysr_original']['y2_mse']) / 2
    pysr_original_overall_mape = (results['pysr_original']['y1_mape'] + results['pysr_original']['y2_mape']) / 2
    
    print(f"{'Model':<12} {'Scale':<10} {'Overall_MSE':<15} {'Overall_MAPE':<15}")
    print("-" * 60)
    print(f"{'GINN':<12} {'Scaled':<10} {ginn_scaled_overall_mse:<15.6f} {ginn_scaled_overall_mape:<15.2f}")
    print(f"{'GINN':<12} {'Original':<10} {ginn_original_overall_mse:<15.6f} {ginn_original_overall_mape:<15.2f}")
    print(f"{'PySR':<12} {'Scaled':<10} {pysr_scaled_overall_mse:<15.6f} {pysr_scaled_overall_mape:<15.2f}")
    print(f"{'PySR':<12} {'Original':<10} {pysr_original_overall_mse:<15.6f} {pysr_original_overall_mape:<15.2f}")
    ...
    print("3. Both equations evaluated on scaled and original data")
...
def main():
    ...
    # 2. Apply scaling (avoid zeros for GINN log operations)
    ...
    # Use a range that avoids zeros completely
    scaler_X = MinMaxScaler(feature_range=(0.1, 10.0))  # Avoid zeros, use [0.1, 1.0]
    scaler_Y = MinMaxScaler(feature_range=(0.1, 10.0))  # Avoid zeros, use [0.1, 1.0]
    
    X_scaled = scaler_X.fit_transform(X)
    Y_scaled = scaler_Y.fit_transform(Y)
    
    # Verify no zeros after scaling
    print("After scaling:")
    for i in range(num_features):
    ...
    print(f"Y1: {Y_scaled[:, 0].min():.6f} to {Y_scaled[:, 0].max():.6f}")
    print(f"Y2: {Y_scaled[:, 1].min():.6f} to {Y_scaled[:, 1].max():.6f}")
    ...
    simplified_equations, simplified_predictions = simplify_ginn_equations_with_pysr(
        X_scaled, Y_scaled, ginn_equations, target_names
    )
    
    # 6. Display equations and comprehensive results
    results = display_equations_and_results(
        Y_scaled, Y, ginn_equations, simplified_equations, X_scaled, X, target_names, scaler_Y
    )
    ...
    np.save(os.path.join(npy_dir, 'ginn_original_predictions.npy'), np.column_stack(ginn_predictions))
    if simplified_predictions is not None:
        np.save(os.path.join(npy_dir, 'ginn_simplified_predictions.npy'), simplified_predictions)
    
    print(f"\nPredictions saved to:")
    print(f"  - {npy_dir}/ginn_original_predictions.npy")
    print(f"  - {npy_dir}/ginn_simplified_predictions.npy")
