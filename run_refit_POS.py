#!/usr/bin/env python3
"""
Quick script to test REFITTED equations from POS dataset on test data.
This tests the equations that were refitted on test data (data leakage).
"""
...
def main():
    ...
    print("WARNING: These equations were refitted on test data (DATA LEAKAGE!)")
    print("This is NOT a fair evaluation of generalization!\n")
    
    for target_idx, target_data in enumerate(fold_data['per_target']):
        target_name = target_data['target']
        print(f"--- {target_name.upper()} ---")
        
        # Get the REFITTED equation expression (SCALED version)
        refit_expr = target_data['expr_refit']
        print(f"Refitted Equation (scaled): {refit_expr[:100]}...")
        ...
        # Parse and test the refitted equation
        try:
