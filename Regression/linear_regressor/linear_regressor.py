...
class MultiTaskLinearRegression:
    """
    Multi-task Linear Regression model for predicting multiple target variables.
    """
    
    def __init__(self, random_state: int = 42):
        """
        Initialize the multi-task linear regression model.
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, feature_names: List[str] = None, 
            target_names: List[str] = None) -> 'MultiTaskLinearRegression':
        """
        Fit the multi-task linear regression model.
...
def evaluate_model(model: MultiTaskLinearRegression, X_test: np.ndarray, y_test: np.ndarray, 
                  target_names: List[str]) -> Dict[str, Dict[str, float]]:
    """
    Evaluate the model using MSE and MAPE metrics.
    
    Args:
        model: Fitted multi-task linear regression model
...
def print_evaluation_results(results: Dict[str, Dict[str, float]]):
    ...
    """
    print("\n" + "="*60)
    print("MULTI-TASK LINEAR REGRESSION EVALUATION RESULTS")
    print("="*60)
...
def plot_results(y_true: np.ndarray, y_pred: np.ndarray, target_names: List[str]):
    ...
    for i, target_name in enumerate(target_names):
        ...
        ax.set_title(f'{target_name}: True vs Predicted')
        ax.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig('linear_regressor/linear_regression_results.png', dpi=300, bbox_inches='tight')
    plt.show()

def print_model_equations(model: MultiTaskLinearRegression, feature_names: List[str], 
    ...
    print("\n" + "="*60)
    print("LEARNED LINEAR EQUATIONS")
    print("="*60)
    
    for target_name in target_names:
        print(f"\n{target_name.upper()} = {intercepts[target_name]:.4f}")
...
def main():
    """
    Main function to run the multi-task linear regression analysis.
    """
    ...
    print("\nTraining multi-task linear regression model...")
    ...
    results_df.to_csv('linear_regressor/linear_regression_results.csv')
    print(f"\nResults saved to 'linear_regressor/linear_regression_results.csv'")
