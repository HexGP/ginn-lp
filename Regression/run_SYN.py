...
# Import data utilities for SYN dataset
from data_utils import get_data_utils
data_utils = get_data_utils('SYN')
...
from SYN_Regression.linear_regressor.linear_regressor import MultiTaskLinearRegression
from SYN_Regression.mlp_regressor.mlp_regressor import MLPRegressionModel
from SYN_Regression.gp_regressor.gp_regressor import MultiTaskGaussianProcess
from SYN_Regression.multiout_regressor.multiout_regressor import MultiOutputRegressionModel
from SYN_Regression.sgd_regressor.sgd_regressor import MultiTaskSGDRegressor
...
def save_detailed_results_to_json(all_results: List[Dict], comparison_df: pd.DataFrame, target_names: List[str], output_dir=None):
    ...
    json_output_file = os.path.join(output_dir, 'comparison_result_SYN.json')
...
def main():
    ...
    print("SYN Dataset Model Comparison")
    print("="*50)
    
    # Load and preprocess data
    print("Loading and preprocessing SYN dataset...")
    ...
    script_dir = os.path.dirname(os.path.abspath(__file__))
    output_dir = os.path.join(script_dir, 'all_outputs', 'SYN')
    os.makedirs(output_dir, exist_ok=True)
    
    output_file = os.path.join(output_dir, 'comparison_SYN.csv')
