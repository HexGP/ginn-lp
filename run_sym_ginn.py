...
(one-file runner; MinMax scaling on features fit on train; +1e-6 epsilon)

REGULARIZATION/OPTIMIZATION:
  • Optional noise augmentation (features only)
  • L1/L2 regularization, LR scheduling, early stopping, weight decay

What this does now:
  • Builds/uses your GINN multi-output model (shared PTA layer + 2 heads)
  • Trains with multitask loss (two targets)
  • Every N epochs:
      - Extracts SymPy equations for y1,y2 using the traditional GINN extractor
        (directly from the network’s PTA structure; no gradient/surrogate paths)
      - Optionally refits ONLY numeric coefficients (ridge) with fixed structure
      - Reports R²/MAE/RMSE/MAPE and faithfulness R²(model ↔ equation)

Notes:
  • Inputs are MinMax-scaled (fit on train) with a +1e-6 epsilon; targets unchanged
  • Raw equations are now MTR-style: linear combinations of Laurent/PTA monomials
...
# ---------- Suppress only long recovered-equation prints ----------
@contextmanager
def suppress_long_equation_prints():
    class _StdoutFilter(io.TextIOBase):
        ...
        def write(self, s):
            text = str(s)
            # Start of external long-eq block
            if text.startswith('Recovered equation for output') or text.startswith('Recovered equation'):
                self.suppressing = True
                return len(text)
            # While suppressing, skip lines that look like equation content
            if self.suppressing:
                if ('**' in text and 'X_' in text) or text.strip() == '':
                    # end suppression on blank line
                    if text.strip() == '':
                        self.suppressing = False
                    return len(text)
                # non-equation line encountered → stop suppressing and print it
                self.suppressing = False
            return self.upstream.write(text)
        def flush(self):
            return self.upstream.flush()
    old = sys.stdout
    try:
        sys.stdout = _StdoutFilter(old)
        yield
    finally:
        sys.stdout = old
...
def get_output_filename(dataset_path):
    ...
    return f"outputs/JSON_ENB/ginn_{first_three}_{num_layers}S_{blocks_per_layer}B_{output_blocks}L.json"
...
VALIDATE_EVERY = 400                  # equation sync every N epochs (200 for faster feedback)
ROUND_DIGITS = 3
...
# GINN architecture
LN_BLOCKS_SHARED = (6, )             # 1 shared layer with N PTA blocks
LIN_BLOCKS_SHARED = (1, )            # must match per GINN builder
OUTPUT_LN_BLOCKS = 6                 # PTA blocks in each output head

# Dynamic growth config
MAX_LN_BLOCKS_SHARED = 10            # max PTA blocks per shared layer
MAX_OUTPUT_LN_BLOCKS = 10            # max PTA blocks per output head
GROW_EVERY = 500                     # epochs between growth steps
L1 = 1e-3; L2 = 1e-3
...
def make_vector_fn_debug(exprs, n_features, symbols=None, eps=1e-12, log_every_expr=True):
    ...
    for idx, e in enumerate(exprs):
        ...
        if log_every_expr:
            # Keep logs concise: show type only, not the full expression string
            print(f"[EvalPrep] expr[{idx}] -> {type(e).__name__}")
...
class EquationSyncCallback(Callback):
    def __init__(self, X_train, Y_train, num_features, output_ln_blocks,
        ...
        self.faithfulness_phase = 'faithfulness'
    ...
    def on_train_begin(self, logs=None):
        # Ensure we have initial raw expressions before training starts
        try:
            ...
            self._exprs_raw = exprs
            self._symbols = maybe_syms if isinstance(maybe_syms, (list, tuple)) and len(maybe_syms) else sp.symbols([f"X_{i+1}" for i in range(self.nf)])
            self._f_vec = make_vector_fn_debug(self._exprs_raw, self.nf, symbols=self._symbols, eps=EPS_LAURENT, log_every_expr=False)
            print(f"[EqSync] ✅ Initial raw equations prepared before training")
        except Exception as e:
            print(f"[EqSync] ❌ Failed initial extraction: {e}")
            self._exprs_raw = None
            self._symbols = sp.symbols([f"X_{i+1}" for i in range(self.nf)])
            self._f_vec = None
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        try:
            ...
            print(f"[EqSync] 🎯 Extracting equations directly from network (MTR-style raw)")
            exprs = None
            ...
            try:
                ...
                print(f"[EqSync] ✅ Traditional extractor successful")
                # Summarize equations without printing full forms
                print(f"[EqSync] Summary: expr types={[type(e).__name__ for e in exprs]}")
                # Cache for train_step faithfulness
                self._exprs_raw = exprs
                self._symbols = symbols
                self._f_vec = make_vector_fn_debug(self._exprs_raw, self.nf, symbols=self._symbols, eps=EPS_LAURENT, log_every_expr=False)
            except Exception as e2:
                print(f"[EqSync] ❌ Traditional extraction failed: {e2}")
                # Last resort: create dummy expressions
                exprs = ["<n/a>"] * self.Y_train.shape[1]
            ...
            try:
                y_eq = f_vec(self.X_train)  # [n,2]
                print(f"[EqSync] Equation evaluation successful: y_eq shape = {y_eq.shape}, preview={y_eq[:3].ravel()}")
            except Exception as e:
            ...
            y_pred_list = self.model.predict([self.X_train[:, i].reshape(-1, 1) for i in range(self.nf)], verbose=0)
            y_pred = np.column_stack(y_pred_list)

            # 4) Metrics (pre-refit)
            r2_eq_truth = [r2_score(self.Y_train[:, j], y_eq[:, j]) for j in range(self.Y_train.shape[1])]
            r2_model_truth = [r2_score(self.Y_train[:, j], y_pred[:, j]) for j in range(self.Y_train.shape[1])]
            r2_model_eq = [r2_score(y_pred[:, j], y_eq[:, j]) for j in range(self.Y_train.shape[1])]
            
            # 4.5) Adaptive faithfulness analysis
            current_quality = self.adaptive_system.evaluate_equation_quality(y_eq, self.Y_train, epoch)
            ...
            exprs_refit = refit_coeffs_multi(exprs, self.nf, self.X_train, self.Y_train, symbols=symbols, ridge=self.ridge)
            f_vec_refit = make_vector_fn_debug(exprs_refit, self.nf, symbols=symbols, eps=EPS_LAURENT, log_every_expr=True)
            try:
                y_eq_refit = f_vec_refit(self.X_train)
                print(f"[EqSync] Refit evaluation successful: y_eq_refit shape = {y_eq_refit.shape}, preview={y_eq_refit[:3].ravel()}")
            except Exception as e:
                print(f"[EqSync] Refit evaluation failed: {e}")
                print(f"[EqSync] Refit expression types: {[type(e).__name__ for e in exprs_refit]}")
                raise

            r2_eq_truth_refit = [r2_score(self.Y_train[:, j], y_eq_refit[:, j]) for j in range(self.Y_train.shape[1])]
            r2_model_eq_refit = [r2_score(y_pred[:, j], y_eq_refit[:, j]) for j in range(self.Y_train.shape[1])]
    ...
    def get_raw_equation_fn(self):
        return getattr(self, '_f_vec', None)
...
class FaithfulnessTrainingWrapper:
    """
    Wrapper that adds MTR-style consistency: MSE(y_pred, y_eq_raw) every step (RAW only).
    """
    def __init__(self, base_model, eqsync_callback, alpha=FAITHFULNESS_ALPHA):
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            # Main loss (already set on model)
            if isinstance(y, (list, tuple)) and isinstance(y_pred, (list, tuple)):
                main_loss = tf.constant(0.0, dtype=tf.float32)
                for i in range(len(y)):
                    target_var = tf.math.reduce_variance(y[i]) + 1e-8
                    mse = tf.keras.losses.mse(y[i], y_pred[i])
                    main_loss += mse / target_var
            else:
                main_loss = tf.keras.losses.mse(y, y_pred)

            # RAW equation consistency loss (like MTR: y_pred ≈ y_sym_raw)
            f_vec = self.eqsync_callback.get_raw_equation_fn()
            consistency = tf.constant(0.0, dtype=tf.float32)
            if f_vec is not None:
                # Build batch X matrix from Keras multi-input list x
                if isinstance(x, (list, tuple)):
                    X_batch = tf.concat(x, axis=1)
                else:
                    X_batch = x
                X_np = tf.stop_gradient(X_batch).numpy()
                y_eq_raw_np = f_vec(X_np)
                y_eq_raw = tf.convert_to_tensor(y_eq_raw_np, dtype=tf.float32)
                if isinstance(y_pred, (list, tuple)):
                    for i in range(len(y_pred)):
                        consistency += tf.keras.losses.mse(y_pred[i], y_eq_raw[:, i:i+1])
                else:
                    consistency += tf.keras.losses.mse(y_pred, y_eq_raw)
            
            loss = main_loss + self.alpha * consistency
...
def main():
    ...
    # 2) Train/test split like mtr_ginn.py (seed=42, 80/20 split), no prior shuffle
    X_all = df[feature_cols].values.astype(np.float32)
    Y_all = df[target_cols].values.astype(np.float32)

    tr_idx, te_idx = train_test_split(range(len(X_all)), test_size=0.2, random_state=42)
    fold_splits = [(tr_idx, te_idx)]
    ...
    for fold, (tr, te) in enumerate(fold_splits, 1):
        ...
        # 3) MinMax scaling on features to [0.01, 10] (fit on train); targets unchanged
        print(f"\n🔧 Using MinMax scaling for FEATURES to [0.01, 10] (fit on train)")
        scaler = MinMaxScaler(feature_range=(0.01, 10.0))
        x_train_s = scaler.fit_transform(X_train)
        y_train = Y_train
        x_test_s  = scaler.transform(X_test)
        y_test  = Y_test

        print(f"   x_train_s shape: {x_train_s.shape}, range: [{np.min(x_train_s):.6f}, {np.max(x_train_s):.6f}]")
        print(f"   y_train shape: {y_train.shape}, range: [{np.min(y_train):.3f}, {np.max(y_train):.3f}]")

        # 4) Build GINN model with dynamic PTA growth (shared and heads)
        INITIAL_SHARED_BLOCKS = LN_BLOCKS_SHARED[0] if isinstance(LN_BLOCKS_SHARED, (list, tuple)) and len(LN_BLOCKS_SHARED) else int(LN_BLOCKS_SHARED)
        INITIAL_OUTPUT_BLOCKS = int(OUTPUT_LN_BLOCKS)
        current_shared_blocks = max(1, int(INITIAL_SHARED_BLOCKS))
        current_output_blocks = max(1, int(INITIAL_OUTPUT_BLOCKS))

        def build_model(ln_blocks_shared, output_ln_blocks):
            return eql_model_v3_multioutput(
                ...
                output_l1_reg=1e-3, output_l2_reg=1e-3,
            )

        def make_optimizer():
            return eql_opt(decay_steps=DECAY_STEPS, init_lr=INIT_LR)

        base_model = build_model(current_shared_blocks, current_output_blocks)
        ...
        eqsync = EquationSyncCallback(
            ...
            output_ln_blocks=current_output_blocks,
            validate_every=VALIDATE_EVERY,
            round_digits=ROUND_DIGITS,
            ridge_lambda=RIDGE_LAMBDA,
            model=base_model,  # Pass model reference for weight adjustment
            anchor_set=None
        )
        ...
        # 6) Compile with simple variance-normalized multitask MSE (no range/distribution penalties)
        def simple_multitask_loss(task_weights):
        ...
        print(f"\n🎯 COMPILING MODEL WITH SIMPLE MULTITASK LOSS (no range/distribution penalties)")

        base_model.compile(
            optimizer=make_optimizer(),
            loss=simple_multitask_loss(TASK_WEIGHTS),
            jit_compile=False,
        )

        # Quick dry run to confirm forward works
        _ = base_model.predict([x_train_s[:5, i].reshape(-1, 1) for i in range(num_features)], verbose=0)

        # Extra guard: check for additional losses (should now be tensors)
        if base_model.losses:
            print(f"DEBUG: Found {len(base_model.losses)} additional losses")
            print(f"DEBUG: First few loss types: {[type(x).__name__ for x in base_model.losses[:5]]}")
            if len(base_model.losses) > 5:
                print(f"DEBUG: ... and {len(base_model.losses) - 5} more")
        else:
            print("DEBUG: No additional losses found (good!)")

        # Model configuration check
        print(f"DEBUG: Model inputs: {len(base_model.inputs)} inputs with shapes: {[inp.shape for inp in base_model.inputs]}")
        print(f"DEBUG: Model outputs: {len(base_model.outputs)} outputs with shapes: {[out.shape for out in base_model.outputs]}")
        print(f"DEBUG: Model loss function: {base_model.loss}")
        print(f"DEBUG: Model optimizer: {base_model.optimizer}")
        print(f"DEBUG: Model is compiled: {hasattr(base_model, 'loss') and base_model.loss is not None}")
        ...
        print("DEBUG: Starting training with dynamic PTA growth...")
        try:
            epochs_done = 0
            while epochs_done < EPOCHS:
                run_epochs = min(GROW_EVERY, EPOCHS - epochs_done)
                history = base_model.fit(
                    ...
                    epochs=epochs_done + run_epochs,
                    batch_size=BATCH_SIZE,
                    validation_split=VAL_SPLIT,
                    verbose=1,
                    callbacks=[es, es_val_acc, eqsync, lr_scheduler, cosine_scheduler]
                )
                epochs_done += run_epochs

                # Grow after each chunk if limits not reached
                grew = False
                if current_shared_blocks < MAX_LN_BLOCKS_SHARED:
                    current_shared_blocks += 1
                    grew = True
                if current_output_blocks < MAX_OUTPUT_LN_BLOCKS:
                    current_output_blocks += 1
                    grew = True
                if grew and epochs_done < EPOCHS:
                    print(f"\n⏫ Growing PTA blocks → shared={current_shared_blocks}, head={current_output_blocks}")
                    base_model = build_model(current_shared_blocks, current_output_blocks)
                    base_model.compile(
                        optimizer=make_optimizer(),
                        loss=simple_multitask_loss(TASK_WEIGHTS),
                    ...
                    eqsync.model = base_model
                    eqsync.out_ln = current_output_blocks
                    _ = base_model.predict([x_train_s[:5, i].reshape(-1, 1) for i in range(num_features)], verbose=0)
        ...
        # 8) Inference
        nn_pred_list = base_model.predict([x_test_s[:, i].reshape(-1, 1) for i in range(num_features)], verbose=0)
        ...
        try:
            ...
            # RAW: Traditional GINN extraction (MTR-style raw)
            print(f"[Extraction] 🎯 Extracting equations directly from network (MTR-style raw)")
            exprs = None
            Yhat_eq = None
            ...
            try:
                ...
                print(f"[Extraction] ✅ Traditional extractor successful")
            except Exception as e2:
                print(f"[Extraction] ❌ Traditional extraction failed: {e2}")
