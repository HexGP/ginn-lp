...
# =============== USER CONFIG ===============
DATA_CSV = "data/prepared/farmer_resource_optimization.csv"   # <--- farmer resource optimization dataset
...
def get_output_filename(dataset_path):
    ...
    return f"outputs/JSON_farmer_resource/grad_{first_three}_{num_layers}S_{blocks_per_layer}B_{output_blocks}L_shifted.json"

# If you know exact target col names, set them here (otherwise auto-detect below).
TARGET_COLS = ["target_Fertilizer_Usage_kg", "target_Pesticide_Usage_kg"]  # Farmer resource optimization targets
...
def main():
    # 1) Load data (farmer resource optimization dataset)
    df = pd.read_csv(DATA_CSV)
    # Drop NULLs (if any)
    df.dropna(inplace=True)
    
    # No categorical target conversion needed - all targets are continuous
    print("✅ All targets are continuous - no conversion needed")
    ...
    # Verify targets are numeric
    for col in target_cols:
        if not pd.api.types.is_numeric_dtype(df[col]):
            print(f"⚠️ Warning: Target '{col}' is not numeric, converting...")
    ...
    # 12) Save results
    os.makedirs("outputs/JSON_farmer_resource", exist_ok=True)
