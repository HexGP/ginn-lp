...
class MultiTaskLinearRegression:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, feature_names: List[str] = None, 
        ...
        # Fit separate linear regression model for each target
    ...
    def get_coefficients(self) -> Dict[str, np.ndarray]:
        ...
        for target_name, model in self.models.items():
            coefficients[target_name] = model.coef_
    ...
    def get_intercepts(self) -> Dict[str, float]:
        ...
        intercepts = {}
        for target_name, model in self.models.items():
            intercepts[target_name] = model.intercept_
...
def main():
    ...
    for i, target_name in enumerate(target_names):
        ...
        mse_scores = cross_val_score(
            LinearRegression(), X, y[:, i], 
            cv=5, scoring='neg_mean_squared_error'
        )
        cv_scores_mse[target_name] = -mse_scores.mean()
        
        # MAPE cross-validation (using custom scoring)
