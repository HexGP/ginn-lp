...
class MultiTaskGaussianProcess:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, feature_names: List[str] = None, 
        ...
        # Fit separate Gaussian Process model for each target
    ...
    def get_kernels(self) -> Dict[str, object]:
        ...
        for target_name, model in self.models.items():
            kernels[target_name] = model.kernel_
    ...
    def get_log_marginal_likelihood(self) -> Dict[str, float]:
        ...
        log_likelihoods = {}
        for target_name, model in self.models.items():
            log_likelihoods[target_name] = model.log_marginal_likelihood()
...
def main():
    ...
    for i, target_name in enumerate(target_names):
        ...
        mse_scores = cross_val_score(
            GaussianProcessRegressor(kernel=best_model.get_kernels()[target_name], random_state=42),
            X, y[:, i], 
    ...
    results_df.to_csv('gp_regression_results.csv')
    print(f"\nResults saved to 'gp_regression_results.csv'")
